...
function withTimeout<T>(work: Promise<T>, onTimeout: () => void): Promise<T> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      onTimeout();
      reject(new Error("The capture did not finish in time."));
    }, RENDER_TIMEOUT_MS);
...
async function captureSnapshot(
  ...
  const [{ toBlob, toSvg }, fontEmbedCSS] = await Promise.all([
    ...
    fontEmbedCss(FONTS[settings.font], signal),
  ...
  if (format === "svg") {
    const dataUrl = await toSvg(snapshot, options);
  ...
  const blob = await toBlob(snapshot, options);
...
export async function renderImage({ node, settings, format, scale }: RenderRequest): Promise<Blob> {
  ...
  try {
    return await withTimeout(
      captureSnapshot(snapshot, settings, format, scale, controller.signal),
      () => controller.abort(),
    );
  } finally {
    snapshot.remove();
